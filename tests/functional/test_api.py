...
def test_upload_cube(client):
    files = [
        ...
        for file in listdir(TEST_CUBE)
        if isfile(join(TEST_CUBE, file))
    ]

    response = client.post("/api/cubes/add", data={"files": files})
    result = response.get_json()
    assert sorted(result["dimensions"]) == sorted(
        [
            "tests/demo_csv_cubes/sales/Time.csv",
            "tests/demo_csv_cubes/sales/Facts.csv",
            "tests/demo_csv_cubes/sales/Product.csv",
            "tests/demo_csv_cubes/sales/Geography.csv",
        ]
    )
